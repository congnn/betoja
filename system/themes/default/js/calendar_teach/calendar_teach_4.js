$(document).ready(function() {
    ...
    $('.e_table_data').find('.e_teacher').each(function() {
    ...
    $(document).on('change', '.e_check_input', function() {
        if (isNaN($(this).val())) {
    ...
    $(document).on('change', '.e_typeClass', function() {
        var levelClass = $('.e_levelClass').find('select.levelClass');
        if ($(this).val() == 2) {
            $('.e_levelClass').show();
            levelClass.removeAttr('disabled');
        }
        if ($(this).val() == 1) {
            $('.e_levelClass').hide();
            levelClass.attr('disabled');
        }
    });
...
//$('#arrange_filter_form').submit(function (){
//        e.preventDefault();
...
function dropHandler(target, e) {
    ...
    if ($(target).hasClass(tagert_class)) {
        if ($(target).hasClass('e_info_teacherBackup')) {
            target.appendChild(document.getElementById(id));

            $('#' + id).parents('.e_table_data').find('.' + tagert_class).each(function() {
                var teacherBackup = $(this).find('.teacherBackup');
                var current_val = teacherBackup.val();
                if (current_val) {
                    current_val += ';' + teacher_id;
                } else {
                    current_val += teacher_id;
                }
