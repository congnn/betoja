...
abstract class data_base extends CI_Model {
     ...
     * @return Array        Mảng các object tương tự $schema
     */
    function get_list_table($search_text = "", $whereCondition = NULL, $limit = 0, $post = 0, $order = NULL, &$total = 0) {
        $this->setting_select();
        ...
        if (is_array($where) && strlen($search_text)) {
            ...
            foreach ($where as $key => $value) {
                ...
                if ($rule) {               
                    ...
                    if (isset($rule['real_field'])) {/* Nếu rule có real_field thì lấy key ở real_fiel */
                        $x_key = $rule['real_field'];
                    }

                    if (isset($rule['disable_search'])) {
                        $x_key = FALSE;
                    }
                }
                if ($x_key !== FALSE) {
    ...
    function get_display_rule($rule) {
        $string = '';
        if ($rule && count($rule)) {
