 ...
 * @version 0.0.0
 */
abstract class admin_manager_base extends admin_home_base {
    ...
    public function __construct() {
        ...
        $this->load->model('admin/'.$this->name["model"], "data");
        $this->url["add"] = site_url('admin/'.$this->name["class"] . "/add");
        $this->url["view"] = $this->name["class"] . "/view/";
        $this->url["edit"] = $this->name["class"] . "/edit/";
        $this->url["delete"] = $this->name["class"] . "/delete/";
        $this->url["manager"] = site_url('admin/'.$this->name["class"] . "/manager");
        $this->url["search"] = site_url('admin/'.$this->name["class"] . "/search");
    }
    ...
    public function add($data = Array()) {
        ...
        if (!isset($data["save_link"])) {
            $data["save_link"] = site_url('admin/'.$this->name["class"] . "/add_save");
        }
    ...
    public function edit($id = 0, $data = Array()) {
        ...
        if (!isset($data["save_link"])) {
            $data["save_link"] = site_url('admin/'.$this->name["class"] . "/edit_save/" . $id);
        }
        if (!isset($data["list_input"])) {
    ...
    public function manager($data = Array()) {
        ...
        $data["delete_list_link"] = isset($data["delete_list_link"]) ? $data["delete_list_link"] : site_url('admin/'.$this->url["delete"]);
        $data["ajax_data_link"] = isset($data["ajax_data_link"]) ? $data["ajax_data_link"] : site_url('admin/'.$this->name["class"] . "/ajax_list_data");
        $data["title"] = isset($data["title"]) ? $data["title"] : "Quản lý " . $this->name["object"];
        ...
        $title = "Quản lý " . $this->name["object"];
        $this->master_page($content, $head_page, $title);
    }
    ...
    public function view($id = 0, $data = Array()) {
        ...
        if (!isset($data["save_link"])) {
            $data["save_link"] = site_url('admin/'.$this->name["class"] . "/edit_save");
        }
...
//                                $data[$key] = md5($this->md5_salt . sha1($data[$key] . $this->md5_salt));
    ...
    protected function _process_data_table($record) {
        ...
        if (is_array($record)) {
			...
			<a class="detail e_ajax_link icon16 i-eye-3 " per="1" href="' . site_url('admin/'.$this->url["view"] . $record->$key_table) . '" title="Xem"></a>';
            if (!isset($record->editable) || (isset($record->editable) && $record->editable)) {
                $record->custom_action .= '<a class="edit e_ajax_link icon16 i-pencil" per="1" href="' . site_url('admin/'.$this->url["edit"] . $record->$key_table) . '" title="Sửa"></i></a>
			<a class="delete e_ajax_confirm e_ajax_link icon16 i-remove" per="1" href="' . site_url('admin/'.$this->url["delete"] . $record->$key_table) . '" title="Xóa"></a></div>';
            }
    ...
    protected function _get_form($id = 0) {
        ...
        foreach ($data["field_form"] as $key => $item) {
            if (!isset($data["rule"][$key]) || (isset($data["rule"][$key]['type']) && $data["rule"][$key]['type'] == 'invisible')) { /* Nếu trường ko có rule hoặc có kiểu là hidden */
                continue;
            }
            $temp = new stdClass();
            ...
            if (isset($data["rule"][$key]['type'])) {
                if ($data["rule"][$key]['type'] == 'select') {
                    ...
                    if (isset($list_rule["target_model"]) && isset($list_rule["target_value"]) && isset($list_rule["target_display"])) {
                        ...
                        /**
                         * Sửa đổi việc truyền thêm điều kiên vào câu lệnh get_list_option
                         * Date: 20/10/2014
                         * Author: locmx
                         */
                    ...
                    $data["rule"][$key] .= " preview_url=" . site_url('admin/'.$this->name["class"] . "/preview_image_input");
                    $temp->rule = $data["rule"][$key];
                }elseif ($data["rule"][$key]['type'] == 'file') {
