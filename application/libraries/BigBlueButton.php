...
class BigBlueButton {
    ...
    /**
     *
     * @param type $config = Array("bbb_url" => "", "bbb_salt" => "");
     */
    function __construct($config) {
        /*
          Establish just our basic elements in the constructor:
         */
        $this->_securitySalt = $config["bbb_salt"];
        $this->_bbbServerBaseUrl = $config["bbb_url"];
    }

    public function setConfig($salt, $url) {
    ...
    //TPE addition api
    function customApiUrl($api, $params, $apiF = "api") {
        $urlParams = Array();
        $creationUrl = $this->_bbbServerBaseUrl . $apiF . "/" . $api . "?";
    ...
    function customApiRequest($api, $params) {
        $data = $this->_processXmlResponse($this->customApiUrl($api, $params));
//        $url = $this->customApiUrl($api, $params);
//        $xml = '';
//        $ch = curl_init() or die(curl_error());
//        $timeout = 10;
//        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
